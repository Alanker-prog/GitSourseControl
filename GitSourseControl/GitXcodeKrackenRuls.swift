...
/*
 ✴️ Ветки
 ❌ СОХРАНЕНИЕ ИЗМИНЕНИЙ Option + Command + C
 ❌ Перед созданием или переходом на новую ветку КОМИТТИМ ВСЕ ИЗМИНЕНИЯ И ЗАПУШИВАЕМ ИХ на Git (в противном случае они сохраняться в тайник, а если закомитиш в новой ветке эти изминения попадут в обе ветки и будет конфликт версий‼️).Далее закрываем все View(окна) из ветки на котрой работали до прехода на новую ветуку. (РАЗДЕЛЯЕМ View ДЛЯ РАЗДНЫХ ВЕТОК)‼️
 ❌ Если работаешь в команде над проэктом, но обособлено только над своей частью проэкта нужно создать для себя отдельную папку со своими иницыалами. Как это сделать -> Жмем на main правой кнопкой New Branch from "main".. далее в окне для имени ветки пишем сначала свои инициалы затем косая черта и далее имя ветки, ПРИМЕР -> al/add_some_code (ТО ЧТО НАПИСАННО ДО КОСОЙ ЧЕРТЫ al ТАК И БУДЕТ НАЗЫВАТЬСЯ ПАПКА В КОТОРОЙ БУДУТ ХРАНИТЬСЯ ТВОИ ВЕТКИ, В ЭТОЙ ПАПКЕ МОЖНО СОЗДОВАТЬ ПОДВЕТКИ НО ПРИ СОЗДАНИИ НОВОЙ ВОДВЕТКИ НУЖНО ТАКЖЕ УКАЗАТЬ В ЕЕ ОБЯЗАТЕЛЬНО ИМЕНИ ИМИ ПАПКИ  al/add_some_code). В Krackenе не отображаются эти папки, только ветви!
 ❌ Если нужно создать папку в папке -> также как обычно создаешь ветку но имя ветки пишешь следующим образом -> feature/new_onboarding/add_first_screen (ПЕРВОЕ НАЗВАНИЕ ЭТО БУДЕТ ИМЯ ОСНОВНОЙ ПАПКИ,НАЗВАНИЕ ПОСЕРЕДИНЕ БУДЕТ ИМЯ ПОДПАПКИ,А В КОНЦЕ ЭТО ИМЯ ВЕТКИ.  ТАКОЕ БЫВАЕТ НЕОБХОДИМИ КОГДА ТЫ РАБОТАЕШЬ В КАМАНДЕ НАД ОНОЙ ФУНКУИЕЙ ИЛИ СОВМЕСТНО НАД КАКИМ-ТО КУСКОМ КОДА)
